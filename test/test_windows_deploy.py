...
class WindowsDeployScriptTests(unittest.TestCase):
    ...
    def test_watchdog_checks_local_before_external_and_limits_restarts(self):
        ...
        self.assertIn("ExternalSlowThreshold = 3", watchdog)
        self.assertIn("SlowResponseSeconds = 5", watchdog)
        self.assertIn("CooldownMinutes = 20", watchdog)
        self.assertIn("Measure-ExternalTransfer", watchdog)
        self.assertIn('"Cache-Control" = "no-cache"', watchdog)
        self.assertIn('ExternalProbeUrl = ""', watchdog)
        self.assertIn('"ExternalSlowChecks"', watchdog)
